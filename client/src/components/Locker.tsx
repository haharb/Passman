...
export default function Locker({
  ...
  const { control, register, handleSubmit, reset } = useForm({
    defaultValues: {
      locker: locker.length > 0 ? locker : [{ service: "", login: "", password: "" }],
    },
  });
  ...
  return (
    <FormWrapper
      onSubmit={handleSubmit(({ locker }) => {
        ...
        window.sessionStorage.setItem("locker", JSON.stringify(locker));
    ...
    >
        ...
        <Box mt="4" mb="4" display="flex" key={field.id} alignItems="flex-end">
          <FormControl>
            ...
            <Input
              id="service"
              placeholder="Service"
              {...register(`locker.${index}.service`, { required: "The service is required." })}
            />
          </FormControl>
          <FormControl ml="3">
            ...
            <Input
              ...
              {...register(`locker.${index}.login`, { required: "The login credential is required." })}
            />
          </FormControl>
          <FormControl ml="3">
            ...
            <Input
              ...
              {...register(`locker.${index}.password`, { required: "The Password is required." })}
            />
          </FormControl>
          <Button
            type="button"
      ...
      <Button onClick={() => append({ service: "", login: "", password: "" })}>Add</Button>

      <Button ml="8" color="white" background="green" type="submit">
        Save
      </Button>
